...
/**
 * An extension of the native Java <code>Observable</code> class that addresses
 * the need for explicit disposure arising from the circular references inherent
 * to the observer pattern (observer knows observable and vice versa). In a
 * multi-layer observer architecture, the disposure command has to be passed way
 * up the observer hierarchy to break the circular references in every layer and
 * thus make the participants amenable for garbage collection.
 */
public class ExtObservable extends Observable implements Disposable {
     ...
     * <code>Change_ObservableGone</code> with myself as the item is sent out to
     * all of my observers. Subclasses may override this to break their
