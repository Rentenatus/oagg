...
// + SimpleVariableOrder used for ordering of internal SortedSeasonSets.
// Tested.
//
 ...
 */
public class Search_BreadthFirst1 implements SearchStrategy {

    public Search_BreadthFirst1() {
    }
     ...
     */
    private final Vector<Query> bfs(SortedSeasonSet<Variable> breadthvars,
            SortedSeasonSet<Variable> varsleft) {
        if (!breadthvars.isEmpty()) {
            Vector<Query> aQueryList = new Vector<Query>();
            SortedSeasonSet<Variable> aVarSet = new SortedSeasonSet<>(SimpleBiPredicateVariable.INSTANCE);
            Variable aVar;
            ArraySeason<Variable> copy = new ArraySeason<>();
            copy.addMovie(breadthvars);
            IteratorWalker<Variable> walker = copy.softWalker();
            while (walker.hasNext()) {
                aVar = (Variable) walker.next();
...
//                SortedSeasonSet<Variable> vic = getVicinity(aVar, varsleft);
//                if (vic != null) {
//                    
    ...
    private final SortedSeasonSet<Variable> getVicinity(Variable v, SortedSeasonSet<Variable> varsleft) {
        ...
        while (allConstraints.hasNext()) {
            aVar = getOtherVariable((BinaryConstraint) allConstraints
                    .next(), v);
            if (varsleft.indexOf(0) != -1) {
                if (aVicinity == null) {
                    aVicinity = new SortedSeasonSet<Variable>(SimpleBiPredicateVariable.INSTANCE);
                }
                aVicinity.add(aVar);
            }
        }
