...
//			System.out.println(r.getName());
            final Pair<Boolean, List<String>> ruleRes = this.ruleSequence.getRuleResult(i, r.getName(), ApplicabilityConstants.NO_NODE_DELETING);
            if (ruleRes != null) {
    ...
    private boolean noImpedingPredecessors(
        ...
        for (int i = 1; i < sequence.size(); i++) {
            ...
            final Pair<Boolean, List<String>> ruleRes = this.ruleSequence.getRuleResult(i, ri.getName(), ApplicabilityConstants.NO_IMPEDING_PREDECESSORS);
            if (ruleRes != null) {
    ...
    private DependencyPairContainer makeDependencyPairContainer() {
        PairContainer pc = ParserFactory.createEmptyCriticalPairs(this.gragra,
            ...
            if (!this.gragra.isLayered()
                ...
                if (conflicts != null && !conflicts.isEmpty()) {
                    ...
                    if (indx_r1 >= 0 && indx_r1 >= 0) {
                        for (int i = 0; i < conflicts.size(); i++) {
                            final Pair<OrdinaryMorphism, OrdinaryMorphism> pair = conflicts.get(i).first;
...
//				cr.getRule().getName()+"   depth: "+cr.getDepth());
        ...
        for (int l = 0; l < crSourceList.size(); l++) {
            ...
            int indx = start + l;
            Pair<Boolean, List<String>> ruleRes = this.ruleSequence.getRuleResult(indx, r.getName(), ApplicabilityConstants.NO_IMPEDING_PREDECESSORS);
        ...
        if (listIndx >= crsOfRule.size()) {
            ...
            if (x == 0) {
                ...
                if (crListOfPreRule == null || crListOfPreRule.isEmpty()) {
                    ...
                    final List<List<ConcurrentRule>> conRuleListsOfPreRule = this.getListsOfConcurrentRulesOfRule(preRule, preIndx);
