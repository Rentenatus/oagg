...
public class CriticalPairData {
    ...
    public int getSizeOf(int kind) {
        List<Pair<Pair<OrdinaryMorphism, OrdinaryMorphism>, Pair<OrdinaryMorphism, OrdinaryMorphism>>> list = this.map.get(Integer.valueOf(kind));
...
//				return this.adjustMorph1(this.pair.first.first, this.pair.second.first);
    ...
    private void fillMap(
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            } else if (addToList(p, gname, CriticalPairData.FORBID_PRODUCE_D_TXT, FORBID_PRODUCE_DEPENDENCY)); else if (addToList(p, gname, CriticalPairData.PRODUCE_CHANGE_D_TXT, PRODUCE_CHANGE_DEPENDENCY)); // old code
            else if (addToList(p, gname, CriticalPairData.DELETE_SWITCH_D_TXT, PRODUCE_DELETE_DEPENDENCY)); else if (addToList(p, gname, CriticalPairData.FORBID_SWITCH_D_TXT, FORBID_PRODUCE_DEPENDENCY)); else if (addToList(p, gname, CriticalPairData.CHANGE_SWITCH_D_TXT, PRODUCE_CHANGE_DEPENDENCY));
        }
    }
...
//					System.out.println("READ_DELETE_DEPENDENCY");
            ...
            } else if (kind == CREATE_DELETE_DEPENDENCY) {
                Integer key = Integer.valueOf(kind);
                List<Pair<Pair<OrdinaryMorphism, OrdinaryMorphism>, Pair<OrdinaryMorphism, OrdinaryMorphism>>> l1 = null;
                OrdinaryMorphism m1 = p.first.second;
                OrdinaryMorphism m2 = p.first.first;
