...
public class ALR_InheritCSP extends CSP {
     ...
     */
    final private Dictionary<String, HashSet<GraphObject>> itsTypeMap = new Hashtable<String, HashSet<GraphObject>>();
     ...
     *
     * @param vargraph The ALR graph whose elements represent the variables of
     * the CSP.
     * @param ac The attribute context to map attributes in.
     * @param injective If set to <code>true</code>, only injective solutions
     * will be considered.
     */
...
//	 CSP
    ...
    public void removeObjectNameConstraint(GraphObject anObj) {
        Variable anObjVar = this.itsObjVarMap.get(anObj);
        if (anObjVar != null) {
            Iterator<Object> cons = anObjVar.getConstraints();
            while (cons.hasNext()) {
                Object c = cons.next();
