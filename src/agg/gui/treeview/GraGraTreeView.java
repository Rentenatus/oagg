...
//					this.editorPath));
    ...
    public TreePath getTreePathOfGrammar(final Object elem) {
        for (int i = 0; i < this.tree.getRowCount(); i++) {
            ...
            final GraGraTreeNodeData data = (GraGraTreeNodeData) ((DefaultMutableTreeNode) path.getLastPathComponent()).getUserObject();
            if (data.isGraGra()
                    && (elem == data.getData()
    ...
    public synchronized void resetGraph(final EdGraph graph) {
        ...
        if (selGraGra != null) {
            ...
            if (ndataEdit.isGraph() && ndataSel.isGraph()
                ...
                if (path != null) {
                    GraGraTreeNodeData ndata = (GraGraTreeNodeData) ((DefaultMutableTreeNode) path.getLastPathComponent()).getUserObject();
            ...
            if ((impAsTG && impAsG && impAsR && impAsGC)) {
                ...
                switch (answer) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        importAsGraphConstraints(importGraGra, path);
                        break;
                    case 4:
                        importAsGrammarItems(importGraGra, path);
                        break;
                }
...
//				selGraGra.getBasisGraGra().addRuleSubset(set); // test
    ...
    private boolean importAsGraphConstraints(final EdGraGra importGraGra, final TreePath treepath) {
        ...
        if (path != null) {
            ...
            switch (answer) {
                case 0:
                    importAllGraphConstraints(importGraGra, selGraGra);
                    break;
                case 1:
                    importAsAtomicGraphConstraints(importGraGra, path);
                    break;
            }
            return true;
        }
