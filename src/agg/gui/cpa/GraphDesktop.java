...
//				int w = this.nextX + graphFrame.getDesktopIcon().getSize().width;
    ...
    public JInternalFrame addRule2(Rule rule, int w1, int h1) {
        if ((this.option == null)
                || (this.option.getNumberOfCriticalPair() <= this.desktop.getComponentCount()) //				|| (rule instanceof RuleScheme)
                ) {
            return null;
        }
     ...
     * @param first the first rule
     * @param second the second rule
     * @param text text to show in the frame
     */
     ...
     *
     * @param graph The new overlapping graph.
     * @param pair A pair of morphisms
     */
...
//				}
    ...
    public void internalFrameActivated(InternalFrameEvent e) {
        ...
        if (jif == this.cpaGraphFrame
            ...
            GraphEditor gege = (GraphEditor) c;
            EdGraph eg = gege.getGraph();
            Pair<Pair<OrdinaryMorphism, OrdinaryMorphism>, Pair<OrdinaryMorphism, OrdinaryMorphism>> morphs = this.overlappings.get(eg.getBasisGraph());
            if (morphs == null) {
                return;
            }
