 ...
 * contains: type editor, rule editor and graph editor. It provides menus: Edit, Mode, Transform.
 *
 * @author $Author: olga $
 * @version $ID:$
 */
...
public class GraGraEditor extends JPanel implements TreeModelListener,
    ...
    /**
     * Creates a gragra editor containing type, rule, graph, attribute, input parameter editors and transformation
     * component
     */
...
//		System.out.println("##########   GraGraEditor.tableChanged of attributes:: ");
    ...
    /**
     * Returns my active panel. There are four panels: graph, left rule side, right rule side, NAC
     */
...
//								i--;
    ...
    /**
     * Allows replace nodes by icons, if the node type has an icon representation.
     */
...
//					this.ac = (EdNestedApplCond) data.getRule().getNestedACs().firstElement();
    ...
    private EdAtomApplCond loadPostApplCondOfRuleInEditor(final GraGraTreeNodeData data) {
        ...
        if (node.getChildCount() != 0) {
            ...
            if (this.gragra != null) {
                this.gra = ((GraGraTreeNodeData) ((DefaultMutableTreeNode) node.getParent().getParent()).getUserObject())
                        .getGraGra();
    ...
    private EdAtomApplCond loadAtomicOfPostAplCondOfRuleInEditor(final GraGraTreeNodeData data) {
        ...
        if (this.gragra != null) {
            this.gra = ((GraGraTreeNodeData) ((DefaultMutableTreeNode) node.getParent().getParent().getParent())
                    .getUserObject()).getGraGra();
            this.rule = ((GraGraTreeNodeData) ((DefaultMutableTreeNode) node.getParent().getParent()).getUserObject())
                    .getRule();
            if (this.gra != this.gragra) {
                this.resetGraGra = true;
                this.graph = this.gra.getGraph();
